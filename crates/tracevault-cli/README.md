...
register one. Populate it with `tracevault context set --user flow=… label=…` (works with no
checkout). Pass `--no-user-context` to `init --global` to skip enabling it, or `--user-context
